...
2. Descarga `Pokemon-Z-Mods-v1.1.2.zip` de la release y descomprímelo.
...
2. Descomprime `Pokemon-Z-Mods-v1.1.2.zip`.
...
Extract `Pokemon-Z-Mods-v1.1.2.zip`, double-click `Install Pokemon Z Mods.cmd`, and select the folder that directly contains `Game.exe`. The installer does not require administrator privileges, never edits `Data/Scripts.rxdata`, and backs up `preload.rb` and `mkxp.json`.
...
Décompressez `Pokemon-Z-Mods-v1.1.2.zip`, double-cliquez sur `Install Pokemon Z Mods.cmd`, puis sélectionnez le dossier contenant directement `Game.exe`. L'installateur ne demande pas de droits administrateur, ne modifie jamais `Data/Scripts.rxdata` et sauvegarde `preload.rb` et `mkxp.json`.
